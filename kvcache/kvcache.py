...
def get_states_size(config: ModelConfig):
    conv_state_size = (config.linear_num_value_heads * config.linear_value_head_dim + 2 * config.linear_num_key_heads * config.linear_key_head_dim) * 2
    ssm_state_size = config.linear_num_value_heads * config.linear_key_head_dim * config.linear_value_head_dim * 4
    states_size = config.num_linear_attn_layers * (
        conv_state_size + ssm_state_size
    )
    return states_size
